...
def startSTT(sttMode, sampleFmt, sampleRate, channel, callback):
    ...
    msgPayload = 'CODE0200:'
    if sttMode != 'NULL':
        msgPayload += 'MODE=' + sttMode + ','
    msgPayload += 'F=' + sampleFmt + ',R=' + str(sampleRate) + ',C=' + str(channel)
    print(msgPayload)
    sendMessage(msgType, msgPayload)
    onErrorCallback = callback

def startSTT2(sttModelCode, callback):
    global onErrorCallback
    msgType = 'sttStartCmd'
    msgPayload = 'CODE0200:'
    if sttModelCode != 'NULL':
        msgPayload += 'ModelCode=' + str(sttModelCode) 
    sendMessage(msgType, msgPayload)
    onErrorCallback = callback
...
def setAuthParams(x_client_key, x_auth_timestamp, x_client_signature):
    grpc_channel.X_CLIENT_KEY = x_client_key
    grpc_channel.X_AUTH_TIMESTAMP = x_auth_timestamp
    grpc_channel.X_CLIENT_SIGNATURE = x_client_signature
        ...
        if responses.HasField("signal"):
            ...
            if mType == 'sttStartRes':
                ...
                if errorCode == 'CODE0301':
                    ...
                    strEntryPoint = HttpUtils.setGrpcEntrypoint(strEntrypoint)
